...
class sc_fwd_maturity(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        ret = self.nodes[1].sc_create(123, "dada", creation_amount, proving_system, mcTest.generate_params("sc2"), "", generate_random_field_element_hex())
        scid_2 = ret['scid']
        mark_logs("created SC id: {}".format(scid_2), self.nodes, DEBUG_MODE)

        ret = self.nodes[1].sc_create(123, "dada", creation_amount, proving_system, mcTest.generate_params("sc3"), "", generate_random_field_element_hex())
        scid_3 = ret['scid']
        mark_logs("created SC id: {}".format(scid_3), self.nodes, DEBUG_MODE)

        ret = self.nodes[1].sc_create(123, "dada", creation_amount, proving_system, mcTest.generate_params("sc4"), "", generate_random_field_element_hex())
        scid_4 = ret['scid']
