...
void CTxMemPool::DFS(std::vector<std::pair<std::string, std::string> > graph, std::string node, std::map<std::string, bool> * visited, std::stack<std::string> * order) {
...
std::list<uint256> CTxMemPool::TopologicalSort() {
	std::vector<std::pair<std::string, std::string> > dependencyGraph;
...

	{
		...
		//Topologic-order
		for(std::map<std::string, bool >::const_iterator it = visited.begin();
			it != visited.end(); ++it)
		{
			if (it->second == false) {
