...
TEST_F(SidechainMultipleCertsTestSuite, GetTopQualityCertFromDbIsNull) {
    ...
    CSidechain sidechain;
    sidechain.lastCertificateQuality = 100;
    sidechain.lastCertificateHash = uint256S("aaa");
    sidechain.lastEpochReferencedByCertificate = 15;
    ...
    EXPECT_TRUE(aChainStateDb.GetTopQualityCert(scId, sidechain.lastEpochReferencedByCertificate, retrievedCertHash)
...
TEST_F(SidechainMultipleCertsTestSuite, GetTopQualityCertFromViewSameEpoch) {
    ...
    uint256 retrievedCertHash;
    EXPECT_TRUE(sidechainsView->GetTopQualityCert(scId, sidechain.lastEpochReferencedByCertificate, retrievedCertHash)
                    == CScCertificate::QUALITY_NULL);
    ...
    EXPECT_TRUE(sidechainsView->GetTopQualityCert(scId, sidechain.lastEpochReferencedByCertificate, retrievedCertHash)
                    == sidechain.lastCertificateQuality);
    EXPECT_TRUE(retrievedCertHash == sidechain.lastCertificateHash);
}
...
TEST_F(SidechainMultipleCertsTestSuite, GetTopQualityCertFromViewDifferentEpochAreNotReturned) {
    ...
    EXPECT_TRUE(sidechainsView->GetTopQualityCert(scId, sidechain.lastEpochReferencedByCertificate +1, retrievedCertHash)
    				== CScCertificate::QUALITY_NULL);
    EXPECT_TRUE(retrievedCertHash.IsNull());
...
TEST_F(SidechainMultipleCertsTestSuite, GetTopQualityCertFromViewMempool_CertInBackingViewOnly_SameEpoch) {
    ...
    // Non-null quality after flushing the sidechain
    EXPECT_TRUE(viewMempool.GetTopQualityCert(scId, sidechain.lastEpochReferencedByCertificate, retrievedCertHash)
                    == sidechain.lastCertificateQuality);
    EXPECT_TRUE(retrievedCertHash == sidechain.lastCertificateHash);
}
...
TEST_F(SidechainMultipleCertsTestSuite, GetTopQualityCertFromViewMempool_CertsInBackingViewAndMempool_SameEpoch) {
	...
	cert.scId = scId;
    cert.quality = sidechain.lastCertificateQuality * 2;
    cert.epochNumber = sidechain.lastEpochReferencedByCertificate;
...
TEST_F(SidechainMultipleCertsTestSuite, GetTopQualityCertFromViewMempool_CertsInBackingViewAndMempool_DifferentEpoch) {
    ...
    EXPECT_TRUE(viewMempool.GetTopQualityCert(scId, sidechain.lastEpochReferencedByCertificate+1, retrievedCertHash)
    			== CScCertificate::QUALITY_NULL);
    EXPECT_TRUE(retrievedCertHash.IsNull());
