...
BOOST_AUTO_TEST_CASE(util_ParseParameters)
{
    ...
    BOOST_CHECK(mapArgs.empty() && getMapMultiArgs().empty());
    ...
    BOOST_CHECK(mapArgs.size() == 3 && getMapMultiArgs().size() == 3);
    BOOST_CHECK(mapArgs.count("-a") && mapArgs.count("-b") && mapArgs.count("-ccc")
                && !mapArgs.count("f") && !mapArgs.count("-d"));
    BOOST_CHECK(getMapMultiArgs().count("-a") && getMapMultiArgs().count("-b") && getMapMultiArgs().count("-ccc")
                && !getMapMultiArgs().count("f") && !getMapMultiArgs().count("-d"));

    BOOST_CHECK(mapArgs["-a"] == "" && mapArgs["-ccc"] == "multiple");
    BOOST_CHECK(getMapMultiArgs()["-ccc"].size() == 2);
}
